...
{
    if(join){
        err_t err =igmp_joingroup(IP4_ADDR_ANY, addr);
        if (err) {
            LT_IM(LFS,"UDP igmp_joingroup FALSE, err =  %d", (int)err);
            return ESP_ERR_INVALID_STATE;
        }
    } else {
        if (igmp_leavegroup(IP4_ADDR_ANY, addr)) {
            LT_IM(LFS,"UDP igmp_leavegroup FALSE");
            return ESP_ERR_INVALID_STATE;
        }
    }
...
bool AsyncUDP::listenMulticast(const ip_addr_t *addr, uint16_t port, uint8_t ttl)
{
    ...
    if (joinMulticastGroup(addr, true)!= ERR_OK) {
        LT_IM(LFS,"UDP joinMulticastGroup FALSE");
        return false;
    }

    if(!listen(NULL, port)) {
        LT_IM(LFS,"UDP listen FALSE");
        return false;
    }
